...
Команда `/admin` открывает панель с Premium-эмодзи и цитатами. Для обычных пользователей эта команда остаётся без ответа; кнопок настроек у них нет. Администраторы видят кнопку **Настройки** в главном меню и под результатами. Первоначальный вход в админ-панель — через `/admin`; из её разделов можно вернуться кнопкой **Назад**. Если настройки открыты из обычного меню, кнопка возврата ведёт в обычное меню. Язык интерфейса определяется по языку Telegram: русский или английский, для остальных языков — русский.

В панели доступны четыре раздела:

- **Настройки** — Общий формат новых ответов обычным пользователям, кроме администраторов;
- **Администраторы** — Добавление и отзыв доступа;
- **Экспорт настроек** — Файл `result-settings.json` с общими параметрами оформления для сравнения или ручного восстановления; без изменения текущих настроек. Это не полная копия базы, автоматического импорта нет;
- **Состояние бота** — Время работы процесса, режим Long polling, доступность базы настроек для чтения, число администраторов, активные текстовые запросы и их лимит, пауза между паками одного пользователя, заполнение и срок хранения TXT-кэша, версии Python и aiogram. Кнопка **Обновить** запрашивает свежий снимок показателей.
...
Настройки разделены на независимые профили:

- **Главное меню → Настройки** — Личный формат текущего администратора. Не влияет на других администраторов и обычных пользователей;
- **Админ-панель → Настройки** — Общий формат только для обычных пользователей. Администраторов не затрагивает.

Личные настройки каждого администратора сохраняются после перезапуска. Новый личный профиль начинается со стандартного формата `emoji - ID`, независимо от общего профиля. После отзыва прав используется общий формат, а личные настройки удаляются; для ID, удалённых из `.env`, очистка выполняется при следующем запуске.

В каждом профиле можно изменить:
...
Изменения не переформатируют уже отправленные сообщения и готовые TXT-файлы. Кнопка сброса возвращает значения по умолчанию только в открытом профиле, не затрагивая остальные профили и список администраторов. Экспорт из админ-панели содержит общий профиль обычных пользователей, а не личные настройки администратора.
...
| `DB_PATH` | Путь к SQLite-базе общих и личных настроек администраторов и их доступа; по умолчанию `data/bot.db`, относительно папки запуска |
...
Команда `/settings` доступна только администраторам и открывает их личные настройки результата, а не админ-панель. Для остальных пользователей она остаётся без ответа. В обычном списке команд отображаются только `/start` и `/help`.
...
При обновлении версии с уже существующими общими настройками они сохраняются и продолжают действовать для обычных пользователей. Для администраторов добавляется отдельная таблица личных настроек с независимыми значениями по умолчанию. Старые кнопки изменения формата без указания профиля больше не применяются: откройте нужные настройки заново.
