...
Команда `/admin` открывает панель с Premium-эмодзи, цитатами и двумя разделами: **Настройки** и **Администраторы**. Обычные пользователи сразу получают результат и не видят кнопок настроек. Язык интерфейса определяется по языку Telegram: русский или английский, для остальных языков — русский.
...
Старая команда `/settings` также доступна только администраторам и открывает админ-панель. В обычном списке команд отображаются только `/start` и `/help`.
